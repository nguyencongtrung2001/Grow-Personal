...
/**
 * Notes Route — Error Boundary Segment.
 */
export default function NotesError({ error, reset }: ErrorBoundaryProps) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="max-w-md w-full text-center space-y-6">
        <div className="mx-auto w-14 h-14 rounded-2xl bg-violet-50 border border-violet-100 flex items-center justify-center shadow-sm">
          <AlertTriangle className="w-6 h-6 text-violet-600" aria-hidden="true" />
        </div>
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">
            Your notes and documents couldn&apos;t be loaded.
            All content is safely backed up.
          </p>
          {error.digest && (
